...
- **D-01 | Isolation des Groupes de Sécurité (SG)**: Implémentation du modèle "3-Tiers" standard :
  ...
  - `db_sg` (Privé) : Ouvert uniquement au trafic provenant du `app_sg` sur le port de la base de données (configurable via une variable `db_port`, au lieu du 5432 par défaut pour plus de discrétion).
