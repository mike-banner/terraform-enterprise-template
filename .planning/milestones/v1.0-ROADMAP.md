...
## Accomplishments
- Mise en place du "Bootstrap" Terraform (Remote State Backend avec S3 et DynamoDB Locking).
- Création du module générique VPC (Couche Réseau `01-network`) avec subnets et NAT gateway, piloté par variables d'environnement.
- Mise en œuvre de l'architecture "3-Tiers" avec 3 Security Groups interdépendants et création de rôles IAM de base.
- Mise en place d'un pipeline CI/CD GitOps via GitHub Actions vérifiant automatiquement le format, la validité et proposant un plan Terraform dans la PR.
