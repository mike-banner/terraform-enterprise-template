...
- **Acceptance Criteria:**
  - [x] Un bucket S3 est configuré avec versioning activé.
  - [x] Une table DynamoDB est configurée pour le State Locking.
...
- **Acceptance Criteria:**
  - [x] Dossier `terraform/01-network` avec VPC, Subnets publics/privés et NAT Gateway.
  - [x] Dossier `terraform/02-security` pour les rôles IAM communs.
  - [x] Les couches supérieures lisent les outputs via `terraform_remote_state`.
...
- **Acceptance Criteria:**
  - [x] Fichiers de variables `dev.tfvars`, `staging.tfvars`, `prod.tfvars` disponibles dans chaque couche.

#### Feature 4: Base GitOps Automatisée
- **Description:** L'exécution de Terraform est garantie par le CI/CD.
- **Acceptance Criteria:**
  - [x] Un workflow (ex: GitHub Actions) génère un `terraform plan` sur les Pull Requests.
